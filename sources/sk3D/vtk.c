/*
Sanjay Kharche.
3D human SAN measurements.
Now the VTK are all binary.
*/
static char help[] = "HSAN VTK maker. 2015 3D SAN. post-processing.\n";
...
// either of the two functions work for floats.
...
// if(argc<1){ printf("tell me number of binary data files.\n"); exit(1); }
// for(file_Counter = 0; file_Counter < 3599; file_Counter++){
for(file_Counter = 0; file_Counter < 10000; file_Counter++){
// file_Counter = atoi(argv[1]); // one file at a time.
...
// measurement 1: Activation pattern as told by Clancy (derivative based) vs as told by Mark (0 crossing)
// the current is already coming in from the simulation.

/**********************************************************************************************/
/**********************************************************************************************/

// since my input to ReverseFloat is element of an array, swapping bytes in the array
// probably changes the data in the rest of the array also.
	for(usr_k = 0; usr_k < usr_MZ; usr_k++)
	for(usr_j = 0; usr_j < usr_MY; usr_j++)
	for(usr_i = 0; usr_i < usr_MX; usr_i++)
u2[usr_k][usr_j][usr_i] = ReverseFloat(usr_u0_loc[usr_k][usr_j][usr_i]);

		sprintf(str,"HumanSAN%06d.vtk",file_Counter);
		output = fopen(str,"wb");
		...
		fwrite(u2,sizeof(float),usr_MX*usr_MY*usr_MZ,output);
		fclose(output);
...
/*
	...
	for(usr_j = 0; usr_j < usr_MY; usr_j++){
	for(usr_i = 0; usr_i < usr_MX; usr_i++)
		fprintf(output,"%f ",usr_u0_loc[usr_k][usr_j][usr_i]);
	fprintf(output,"\n");
	}
		fclose(output);
*/
