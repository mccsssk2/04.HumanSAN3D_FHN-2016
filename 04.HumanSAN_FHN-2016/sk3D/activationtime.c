...
4 July 2015.
Figure 3:
Col 1: Activation time and leading pacemaker

I moved the ECG and AP calculation to another file.
Col 2: ECG
...
// measurement 1: Activation pattern as told by Clancy (derivative based)
	for(usr_k = 0; usr_k < usr_MZ; usr_k++)
	for(usr_j = 0; usr_j < usr_MY; usr_j++)
	for(usr_i = 0; usr_i < usr_MX; usr_i++){
		if(usr_u0_loc_old[usr_k][usr_j][usr_i]<0.45&&usr_u0_loc[usr_k][usr_j][usr_i]>0.5&&user.geometry[usr_k][usr_j][usr_i][0]>0){
			if(leadingPacemakerLocation<0){ leadingPacemakerLocation = 1; lp_x = usr_i; lp_y = usr_j; lp_z = usr_k;}
if(leadingPacemakerLocation>0&&file_Counter_start<0) file_Counter_start = file_Counter;
if(file_Counter_start>0) activationTime[usr_k][usr_j][usr_i] = (float)(file_Counter-file_Counter_start);
...
// write the activationTime.
		sprintf(str,"activation.vtk");
		output = fopen(str,"w");
		...
		fprintf(output,"SCALARS Act_time float 1\n");
		fprintf(output,"LOOKUP_TABLE default\n");
	for(usr_k = 0; usr_k < usr_MZ; usr_k++)
	for(usr_j = 0; usr_j < usr_MY; usr_j++){
	for(usr_i = 0; usr_i < usr_MX; usr_i++)
		fprintf(output,"%f ",activationTime[usr_k][usr_j][usr_i]);
	fprintf(output,"\n");
	}
		fclose(output);
		...
		sprintf(str,"HumanSANLeadingP.vtk");
		output = fopen(str,"w");
		fprintf(output,"# vtk DataFile Version 1.0\n");
		fprintf(output,"vtk input\n");
		fprintf(output,"ASCII\n");
		...
		fprintf(output,"ORIGIN 0 0 0\n");
		fprintf(output,"POINT_DATA %d\n",usr_MX*usr_MY*usr_MZ);
		fprintf(output,"SCALARS LP float 1\n");
		fprintf(output,"LOOKUP_TABLE default\n");
	for(usr_k = 0; usr_k < usr_MZ; usr_k++)
	for(usr_j = 0; usr_j < usr_MY; usr_j++){
	for(usr_i = 0; usr_i < usr_MX; usr_i++){
		if((usr_i-lp_x)*(usr_i-lp_x)+(usr_j-lp_y)*(usr_j-lp_y)+(usr_k-lp_z)*(usr_k-lp_z)<=9) fprintf(output,"%f ",10.0);
			else fprintf(output,"%f ",-1.0);
		}
	fprintf(output,"\n");
	}
		fclose(output);
